...
const getTodaySummary = async (req, res, next) => {
    try {
        ...
        COUNT(*) AS bill_count
       FROM bills
       WHERE DATE(created_at) = CURRENT_DATE AND payment_status = 'paid'${whereExtra}`, params);
        const existsResult = await db_js_1.default.query(`SELECT id, closed_at FROM cashouts WHERE cashier_id = $1 AND cashout_date = CURRENT_DATE AND status = 'closed' LIMIT 1`, [userId]);
        res.json({
            ...result.rows[0],
...
const createCashout = async (req, res, next) => {
    ...
    try {
        ...
        COUNT(*) AS bill_count
       FROM bills
       WHERE DATE(created_at) = CURRENT_DATE AND payment_status = 'paid' AND cashier_id = $1`, [userId]);
        const s = summaryResult.rows[0];
        const cashoutResult = await client.query(`INSERT INTO cashouts
