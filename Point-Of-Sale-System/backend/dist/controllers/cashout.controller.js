...
const createCashout = async (req, res, next) => {
    try {
        ...
        const s = await db_js_1.default.query(`
      SELECT
    ...
    `).then(r => r.rows[0]);
        const cashoutResult = await db_js_1.default.query(`
    ...
    `, [
        ...
        await db_js_1.default.query('INSERT INTO audit_logs (user_id, action, table_name, record_id, new_values) VALUES ($1,$2,$3,$4,$5)', [userId, 'CASHOUT', 'cashouts', cashout.id, JSON.stringify(cashout)]);
