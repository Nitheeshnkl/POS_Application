...
export const createPurchase = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const purchaseResult = await client.query(
      `INSERT INTO purchases (supplier_name, supplier_phone, invoice_number, purchase_date, total_amount, payment_mode, notes, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [supplier_name, supplier_phone, invoice_number, purchase_date || new Date(), totalAmount, payment_mode, notes, req.user?.id]
    );
    ...
    for (const item of items) {
      ...
      await client.query(
        `INSERT INTO purchase_items (purchase_id, product_id, quantity, unit_price, total_price)
         VALUES ($1, $2, $3, $4, $5)`,
        [purchaseId, product_id, quantity, unit_price, totalPrice]
      );
      ...
      if ((productRes.rowCount ?? 0) > 0) {
        ...
        await client.query(
          ...
          [newStock, unit_price, product_id]
        );
        ...
        await client.query(
          ...
          [product_id, 'purchase', quantity, previousStock, newStock, `Purchase Invoice: ${invoice_number}`, req.user?.id]
        );
      }
    }
...
export const getPurchaseById = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const itemsRes = await pool.query(
      `SELECT pi.*, pr.name_en, pr.unit_type 
